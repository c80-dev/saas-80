...
class PlanAction
{
    ...
    public function all()
    {
      $plans = $this->model->with(['subscribers'])->latest()->paginate(20);
      if (count($plans) < 1) {
        return response()->json([
    ...
    public function get($id)
    {
      ...
      if ($data) {
          $plan = $this->model->with(['subscribers'])->find($id);
    ...
    public function getSinglePlanSubscribers($id)
    {
      $data = $this->model->where('id', '=', $id)->exists();
      if ($data) {
          $plan = $this->model->find($id)->subscribers()->with('user')->get();
          return SubscriberResource::collection($plan);
      }else {
           return response()->json([
               'message' => 'Sorry this data do not exist'
           ], 400);
    ...
    public function update($request, $id)
    {
        ...
        if ($data) {
           ...
           $update = $plan->update([
             ...
             'properties' => empty($request->properties) ? $plan->properties : json_encode($request->properties)
           ]);
           if ($update) {
             return response()->json([
